...
array-bracket-spacing: 배열 안에 공백 유무 (유: [ 'error', 'always' ])
object-curly-spacing: 오브젝트 안에 공백 유무 (유: [ 'error', 'always' ])
...
```
* CRA는 Webpack 설정하려면 eject 해야하지만 craco를 써서 eject 없이 Webpack 설정 한다.
...
```
* CRA에서 baseUrl은 적용 되지만 paths는 적용되지 않는다.
```js
...
### tsconfig.app.json 또는 tsconfig.json 또는 jsconfig.json
```ts
"compilerOptions": {
  "paths": {
    "@/*": ["./src/*"]
...
# React Native
* https://github.com/ovdncids/react-native-curriculum/blob/master/ESLint_Alias.md#eslint
